...
player_x = WIDTH//2
player_y = 20
ball_X = random.randint(200, 500)
ball_y = random.randint(80, 400)
delta_x = random.uniform(2, 3.5)
delta_y = random.uniform(2, 3.5)
points = 0
a = 6
...
def on_update(delta_time):
    global up_pressed, player_y, down_pressed, right_pressed, left_pressed, player_x ,a, player_y, ball_X, ball_y, delta_x, delta_y, asds, points
...
def on_draw():
    ...
    if not asds:
        arcade.draw_text("YOU LOST", WIDTH//2 - 50, HEIGHT//2, arcade.color.RED, 20, align="center")
    ...
    if not new_var and asds:
        arcade.draw_rectangle_outline(WIDTH//2 + 15, HEIGHT//2 + 10, 50, 50, arcade.color.WHITE)
        arcade.draw_text("Start", WIDTH//2, HEIGHT//2, arcade.color.WHITE, 13)
        new_var = True
